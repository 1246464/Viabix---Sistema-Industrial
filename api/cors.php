...
function viabixGetAllowedCorsOrigins() {
    $allowed = [
        ...
        'http://localhost',
        'http://127.0.0.1',
        'file://', // Suporte para apps mobile/desktop
    ];
    ...
    // Adicionar próprio domínio do servidor
    $currentHost = $_SERVER['HTTP_HOST'] ?? null;
    if ($currentHost && !in_array('http://' . $currentHost, $allowed)) {
        $allowed[] = 'http://' . $currentHost;
    }
    if ($currentHost && !in_array('https://' . $currentHost, $allowed)) {
        $allowed[] = 'https://' . $currentHost;
    }
...
function viabixNormalizeOrigin($origin) {
    if (empty($origin)) {
        // Se não houver origin, é uma requisição same-site ou mobile
        return null;
    }
    ...
    // Validar que é uma URL válida ou origin especial
    if ($origin === 'file://' || filter_var($origin, FILTER_VALIDATE_URL)) {
...
function viabixIsOriginAllowed($origin) {
    // Se não houver origin header, é same-site ou app mobile - permitir
    if (empty($origin)) {
        return true;
    }
    ...
    // Verificar match com wildcard (file://)
    if ($normalized === 'file://' && in_array('file://', $allowed, true)) {
        return true;
    }
...
function viabixConfigureCors($allowedMethods = ['GET', 'POST', 'OPTIONS'], $allowedHeaders = ['Content-Type', 'Authorization', 'X-CSRF-Token']) {
    ...
    // Se não houver origin ou origin está permitido, enviar headers CORS
    if (!empty($requestOrigin) && !viabixIsOriginAllowed($requestOrigin)) {
    ...
    if (!empty($requestOrigin)) {
        ...
        // Para requisições sem origin (same-site, mobile), permitir acesso
 ...
 * // Seu código aqui
 */
function viabixHandleCorsPreflight($allowedMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'], $maxAge = 3600, $allowedHeaders = ['Content-Type', 'Authorization', 'X-CSRF-Token']) {
    // Configurar CORS primeiro
    viabixConfigureCors($allowedMethods, $allowedHeaders);
