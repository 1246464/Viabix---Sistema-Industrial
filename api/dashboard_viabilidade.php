...
// Validar tenant_id — não permitir fallback para 'admin'
$tenant_id = viabixCurrentTenantId();
if (!$tenant_id) {
    http_response_code(401);
    exit(json_encode(['erro' => 'Não autenticado ou tenant não identificado']));
}

try {
    $report = [];
    
    // Verificar se o ANVI existe e pertence ao tenant atual
    $countResult = $pdo->prepare("SELECT COUNT(*) as count FROM anvis WHERE tenant_id = ?");
    $countResult->execute([$tenant_id]);
    $count = $countResult->fetch()['count'];
    
    if ($count == 0) {
        http_response_code(404);
        exit(json_encode(['erro' => 'Nenhum ANVI encontrado. Crie um ANVI para visualizar a análise de viabilidade.']));
    }
    ...
    $report['viabilidade'] = [
        ...
        'severidade' => $score_geral >= 80 ? 'success' : ($score_geral >= 60 ? 'warning' : 'error'),
    ...
    $isProduction = (defined('APP_DEBUG') ? !APP_DEBUG : (viabix_env('APP_ENV', 'development') === 'production'));
    echo json_encode([
        'erro' => 'Erro ao processar relatório',
        'mensagem' => $isProduction ? 'Erro interno do servidor' : $e->getMessage(),
